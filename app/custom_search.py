...
def select_date_range():
    ...
    with col2:
        pickup_time = st.selectbox(
            ...
            index=[
                i
                for i, t in enumerate([f"{hour:02d}:00" for hour in range(6, 22)])
                if t == default_time
            ][0],
            key="pickup_time",
        )
    ...
    pickup_datetime_obj = datetime.combine(
        pickup_date, datetime.strptime(pickup_time, "%H:%M").time()
    )
    dropoff_datetime_obj = datetime.combine(
        dropoff_date, datetime.strptime(dropoff_time, "%H:%M").time()
    )
